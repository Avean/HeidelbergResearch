...
module  Nonlinearity
    ...
    function r(Par::Parameters, Var::VariablesVector)
        return Par.Coef.r0 .* (Var.A) ./ (Par.Coef.K .+ Var.Q + Var.A)
    end
...
module Sets
    ...
    function SetTowerRelative(x1, x2, amp)
        ...
        Sets.PerturbationRelative =  VariablesVector(W...)
    end

    function SetTowerAbsolute(x1, x2, val)
        Wa = [getfield(frame_buffer[][1], i) for i in 1:fieldcount(VariablesVector)]
        x1 = round(Int, x1 / SimParam.dx)
        x2 = round(Int, x2 / SimParam.dx)
        for i in 1:fieldcount(VariablesVector)
            Wa[i][x1:x2] .= val
        end
        Sets.PerturbationAbsolute =  VariablesVector(Wa...)
    end

    function PerturbState(U::VariablesVector)
        if SharedState.modify_state_type[] == 'P'
            return U += PerturbationRelative
        elseif SharedState.modify_state_type[] == 'S'
            return U = PerturbationAbsolute
        else
