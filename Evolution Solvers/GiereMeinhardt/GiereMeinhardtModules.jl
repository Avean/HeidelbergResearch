...
module Sets
    ...
    VIni2 = VariablesVector(1.0 .*rand(SimParam.N),
                            1.0 .*rand(SimParam.N));

    VIni3 = VariablesVector(1.77.*ones(SimParam.N),
                            15.7.*ones(SimParam.N));

    Set1 = Parameters(Diffusions(0.0001,1.0), Coefficients(2.0,0.1,0.2)); # Parameters
