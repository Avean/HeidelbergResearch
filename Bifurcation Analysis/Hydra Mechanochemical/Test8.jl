...
function F_hat(U_hat, p)
    ...
    F_hat2 = similar(U_hat)

    # 4. Apply Laplacian in Fourier space
    for s in 1:N_species
        diffcoef = s==bif_param ? p.diffcoef : nu[s]
        F_hat2[:,s] .= laplace_factor .* U_hat[:,s] * diffcoef + Fnl_hat[:,s]
    end
    return F_hat2
end
...
# ---------------------------
function F_flat(U_flat, p)
    # reshape flat vector to (Nk x 5) matrix
    Nk = size(C,2)
    U_hat = reshape(U_flat, Nk, N_species)
    F_flat2 = ones(eltype(U_hat),5*Nk)
    p1 = F_hat(U_hat, p)

    for i in 1:5
        F_flat2[(i-1)*Nk + 1 : i*Nk] = p1[:, 1]   # jeśli chcesz wiersze
    end
    return F_flat2
end
