...
DiffCoef = 1.15;            # start diffusion for bifurcation parameter
N_species = 5;
int_param = [0.5, 3.5];    # Interval in which we consider bifurcation parameter
...
opts_br = ContinuationPar(ds=1e-4, dsmax=5e-3, dsmin=1e-5, p_min=int_param[1], p_max=int_param[2], nev=15, #n_inversion=16,
...
p1 = plot(diagram; markersize=2, title="Bifurcation diagram (Fourier-cosine)", label="", vars = (:param, :nrmReal))
...
plot!(p1, br_missing, vars = (:param, :nrmReal))
###############################################################################################################################
