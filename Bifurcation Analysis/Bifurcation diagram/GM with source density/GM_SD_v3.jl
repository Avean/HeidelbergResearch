...
N_species = 3;
int_param = [0.01, 0.1];    # Interval in which we consider bifurcation parameter
...
function fourier_to_real(U_hat)
    ...
    C_T = T.(C)  # Vorberechnung der Cosinus-Matrix in Typ T
    for s in 1:N_species
        U_real[:,s] = C_T * U_hat[:,s]
    end
    return U_real
end
...
function fourier_to_real_2(U_hat)
    ...
    for s in 1:N_species
        for k = 0:N_modes-1
            ...
            U_real[:,s] .+= U_hat[k+1,s] .* cos.(T(pi)*k .* xT ./ T(L))
        end
    end
    return U_real
end
...
function real_to_fourier(V)
    ...
    T = eltype(V)
    V_four = Array{T}(undef, Nk, N_species)
    C_T = T.(C)
    # --- Least-Squares-Lösung ---
    for s in 1:N_species
        V_four[:,s] = C_T \ V[:,s]   # entspricht np.linalg.lstsq(C, V, rcond=None)[0]
    end
    return V_four
end
...
function test_fourier(V)
    ...
    for s in 1:N_species
        for k in 0:N_fourier
            T = eltype(U_hat)
            xT = T.(x)
            V_rec[:,s] .+= V_hat[k+1,s] .* cos.(T(pi) * k .* xT ./ T(L))
        end
...
function F_nonl_real(U_real)
    S, U, V = U_real[:,1], U_real[:,2], U_real[:,3]
    T = eltype(U_real)
    f1 = U .- S
    f2 = T(param_a) * U .* U .* S ./ (one(T) .+ V) .- T(param_muu) * U
    f3 = T(param_b) * U .* U .* S .- T(param_muuv) * V
...
function F_hat!(F_hat, U_hat, p)
    ...
    lf = T.(laplace_factor)
    for s in 1:N_species
        diffcoef = s == bif_param ? p.diffcoef : T(nu2[s])
        F_hat[:,s] .= lf .* U_hat[:,s] .* diffcoef .+ Fnl_hat[:,s]
    end
    return F_hat
end
...
function F_flat!(F_flat, U_flat, p)
    ...
    F_hat = similar(U_hat, eltype(U_flat))
    F_hat!(F_hat, U_hat, p)
...
# ---------------------------
par_full = (diffcoef = DiffCoef + zero(DiffCoef),)
prob = BifurcationProblem(F_flat!, sol0, par_full, (@optic _.diffcoef);
                           autodiff = true,
                           record_from_solution = (x,p; k...) -> begin
                                T = eltype(x)
                                (
                                    nrmFirst = norm(T.(x[1:Int(end/3)])),
                                    nrmReal = nrm2_real(x,1),
                                    nrm = norm(x),
                                    n∞ = norminf(x[1:Int(end/N_species)]),
                                    sol = x
                                )
                            end,
                           plot_solution = (x, p; k...) -> begin
                            T = eltype(x)
                            C_T = T.(C)
                            plot!(C * x[(bif_param)*Int(end/N_species)+1:(bif_param+1)*Int(end/N_species)] ; k...)
                           end)
# options for Newton solver, we pass the eigen solver
# opt_newton = BK.NewtonPar(tol = 1e-10, max_iterations = 20);
opts_br = ContinuationPar(ds=1e-5, dsmax=5e-3, dsmin=1e-7,
...
p1 = plot(diagram; markersize=2, title="Bifurcation diagram with d_1 = $(nu2[1]), d_3 = $(nu2[3])", label="", vars = (:param, :nrmFirst))
