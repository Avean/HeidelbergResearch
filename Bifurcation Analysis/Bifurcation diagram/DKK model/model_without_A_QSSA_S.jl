...
N_species = 3;
int_param = [0.03, 0.15];    # Interval in which we consider bifurcation parameter
...
function F_nonl_real(U_real)
    ...
    f1 = beta[6]*Wl ./ ((1 .+ C) .* (1 .+ beta[3] .* Wl)) .- Wl
    f2 = beta[2] .* Wl .* Wl .- Wd
    f3 = Wd ./ (1 .+ beta[5] .* Wl) .- C
...
function norminf_all(x)
    return max(norminf_real(x,1), norminf_real(x,2), norminf_real(x,3))
end
...
opts_br = ContinuationPar(ds=1e-5, dsmax=5e-2, dsmin=1e-8,
...
diagram = @time bifurcationdiagram(prob, PALC(), 3, opts_br, normC = norminf_all, bothside=true; verbosity=0, plot=true)
