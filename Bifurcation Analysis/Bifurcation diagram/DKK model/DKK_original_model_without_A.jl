...
# nu2  = [0.0, 0.005, 0.7];
nu2  = [0.0, 6.0*10^(-1), 0.075, 2.0*10^(-4)];
...
N_species = 4;
int_param = [0.02, 0.1];    # Interval in which we consider bifurcation parameter
...
U0 = [0.119582, 7.72763, 3.23776, 0.119582];
U0_real = hcat(U0[1] * ones(Nx), U0[2] * ones(Nx), U0[3] * ones(Nx), U0[4] * ones(Nx));
...
function F_nonl_real(U_real)
    Wl, Wd, C, S = U_real[:,1], U_real[:,2], U_real[:,3], U_real[:,4]
    f1 = beta[6]*S ./ ((1 .+ C) .* (1 .+ beta[3] .* Wl)) .- Wl
    f2 = beta[2] .* Wl .* S .- Wd
    f3 = Wd ./ (1 .+ beta[5] .* Wl) .- C
    f4 = Wl .- S
    return hcat(f1,f2,f3,f4)
end
...
function norminf_all(x)
    return max(norminf_real(x,1), norminf_real(x,2), norminf_real(x,3), norminf_real(x,4))
end
...
prob = BifurcationProblem(F_flat!, sol0, par_full, (@optic _.diffcoef);
                           record_from_solution = (x,p; k...) -> (nrmFirst=norm(x[1:Int(end/N_species)]), nrmReal=nrm2_real(x,1), nrm=norm(x), n∞ = norminf(x[1:Int(end/N_species)]), sol=x),
...
opts_br = ContinuationPar(ds=1e-5, dsmax=5e-2, dsmin=1e-6,
                          p_min=int_param[1], p_max=int_param[2], nev=4*N_fourier, n_inversion=20,
